...
public class DeclareSuperPemOverrideHandler extends ADataHandler {
	...
	protected PlexusConfiguration toConfig(final ConfigurationFactory<PlexusConfiguration> factory,
		...
		if (!(entry instanceof DeclareSuperPemOverride))
			return null;

		final DeclareSuperPemOverride data = (DeclareSuperPemOverride) entry;
		final PlexusConfiguration cnf = factory.create(DeclareSuperPemOverride.HINT);
