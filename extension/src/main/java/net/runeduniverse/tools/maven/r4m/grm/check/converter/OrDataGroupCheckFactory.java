...
import net.runeduniverse.tools.maven.r4m.grm.check.OrCheck;
...
@Component(role = CheckFactory.class, hint = OrDataGroup.CANONICAL_NAME)
public class OrDataGroupCheckFactory extends AGroupCheckFactory {

	@Override
	public Condition<EntityView> createCheck(final DataEntry entry) {
		if (!(entry instanceof OrDataGroup))
			return null;
		final OrDataGroup data = (OrDataGroup) entry;
		final OrCheck check = new OrCheck(data.type());
