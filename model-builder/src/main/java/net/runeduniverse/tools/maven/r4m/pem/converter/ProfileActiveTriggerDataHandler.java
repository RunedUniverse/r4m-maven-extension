...
@Component(role = DataHandler.class, hint = ProfileTriggerActive.CANONICAL_NAME)
public class ProfileActiveTriggerDataHandler extends ADataHandler {

	@Override
	protected PlexusConfiguration toConfig(final ConfigurationFactory<PlexusConfiguration> factory,
			final DataEntry entry) {
		if (!(entry instanceof ProfileTriggerActive))
			return null;

		final ProfileTriggerActive data = (ProfileTriggerActive) entry;
		final PlexusConfiguration cnf = factory.create(ProfileTriggerActive.HINT);
