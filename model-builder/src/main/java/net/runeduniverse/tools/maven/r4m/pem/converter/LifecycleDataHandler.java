...
@Component(role = DataHandler.class, hint = Lifecycle.CANONICAL_NAME)
public class LifecycleDataHandler extends ADataHandler {
	...
	protected PlexusConfiguration toConfig(final ConfigurationFactory<PlexusConfiguration> factory,
		...
		final Lifecycle data = (Lifecycle) entry;
		final PlexusConfiguration cnf = factory.create(Lifecycle.HINT);
		...
		for (Phase phase : data.getPhases()
				.values())
			addConvertedEntry(phaseNodes, factory, phase);
