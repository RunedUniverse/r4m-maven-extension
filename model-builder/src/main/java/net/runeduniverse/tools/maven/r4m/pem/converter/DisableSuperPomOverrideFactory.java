...
@Component(role = DataFactory.class, hint = ModelOverride.CONTEXT + '>' + DisableSuperPomOverride.HINT)
public class DisableSuperPomOverrideFactory extends ADataFactory {

	@Override
	public DataEntry createEntry(final PlexusConfiguration cnf) {
		if (!DisableSuperPomOverride.HINT.equals(cnf.getName()))
			return null;

		Boolean active = getAttributeAsBoolean(cnf, "value", true);
		if (active == null)
			active = true;
		return new DisableSuperPomOverride(active);
	}
}
