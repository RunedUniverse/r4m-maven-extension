...
@Component(role = DataFactory.class, hint = ModelOverride.CONTEXT + '>' + DeclareSuperPemOverride.HINT)
public class DeclareSuperPemOverrideFactory extends ADataFactory {

	@Override
	public DataEntry createEntry(final PlexusConfiguration cnf) {
		if (!DeclareSuperPemOverride.HINT.equals(cnf.getName()))
			return null;

		Boolean active = getAttributeAsBoolean(cnf, "value", true);
		if (active == null)
			active = true;
		return new DeclareSuperPemOverride(active);
	}
}
