...
@Component(role = DataFactory.class, hint = ExecutionRestriction.CONTEXT + '>' + PackagingProcedureRestriction.HINT)
public class PackagingProcedureRestrictionDataFactory extends ADataFactory {

	@Override
	public DataEntry createEntry(final PlexusConfiguration cnf) {
		if (!PackagingProcedureRestriction.HINT.equals(cnf.getName()))
			return null;

		final String procedure = getValueAsId(cnf);
