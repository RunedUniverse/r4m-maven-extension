...
@Component(role = CheckFactory.class, hint = ExecutionData.CANONICAL_NAME)
public class ExecutionCheckFactory implements CheckFactory {
	...
	public Condition<EntityView> createCheck(final DataEntry entry) {
		...
		final ExecutionCheck check = new ExecutionCheck(data.type());
		check.setData(data);
		return check;
	}
}
