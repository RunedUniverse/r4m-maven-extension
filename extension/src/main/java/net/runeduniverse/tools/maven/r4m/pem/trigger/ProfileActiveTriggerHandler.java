...
@Component(role = ExecutionTriggerEvaluationHandler.class, hint = ProfileTriggerActive.CANONICAL_NAME)
public class ProfileActiveTriggerHandler implements ExecutionTriggerEvaluationHandler {
	...
	public boolean isActive(ExecutionArchiveSelectorConfig config, ExecutionTrigger entry) {
		if (!(entry instanceof ProfileTriggerActive))
			return false;
		final ProfileTriggerActive trigger = (ProfileTriggerActive) entry;
